...
namespace API.Controllers
{
    ...
    public class HostVerificationController : ControllerBase
    {
        ...
        [HttpPost("onboard")]
        [Authorize(Roles = "Host")] // Restrict access to hosts
        public async Task<IActionResult> OnboardHost([FromBody] HostOnboardingDto hostDto)
        {
            try
            {
                var stripeAccountId = await _hostVerificationRepository.OnboardHostAsync(
                    hostDto.Email,
                    hostDto.FirstName,
                    hostDto.LastName,
                    hostDto.Country
                ...
                return Ok(new { Message = "Host onboarded successfully.", StripeAccountId = stripeAccountId });
            }
            catch (Exception ex)
            {
