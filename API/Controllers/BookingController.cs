    ...
    public class BookingController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateBooking(int id, [FromBody] BookingInputDTO input)
        {
            ...
            try
            {
                ...
                {
                    UserId = booking.GuestId,
                    Message = $"Your booking for property {property.Result.Title} has been updated.",
                    IsRead = false,
                    CreatedAt = DateTime.UtcNow
                };
                await _notificationRepo.CreateNotificationAsync(notification);
        ...
        public async Task<IActionResult> DeleteBooking(int id)
        {
            try
            {
                ...
                var notification2 = new Notification
                {
                    UserId = (int)booking.Property.HostId,
        ...
        public async Task<IActionResult> CreatePaymentIntent([FromBody] int bookingId)
        {
            try
            {
                var booking = await _bookingRepo.GetByIdWithIncludesAsync(bookingId, b => b.Property.Host);
                if (booking == null)
                    return NotFound("Booking not found.");
                if (booking.Status != BookingStatus.Confirmed.ToString())
                    return BadRequest("Booking must be confirmed before creating a payment intent.");
                var guestId = GetCurrentUserId();
                var hostId = booking.Property.HostId;
                var paymentIntent = await _bookingRepo.CreatePaymentIntentAsync(booking.TotalAmount, guestId, hostId);
